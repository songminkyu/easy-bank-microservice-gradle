...
public class FallbackController {
    ...
    private String determineFailedService(String path) {
        if (path.contains("/eazybank/account")) return "Account service";
        if (path.contains("/eazybank/card")) return "Card Service";
        if (path.contains("/eazybank/loan")) return "Loan service";
        if (path.contains("/v3/api-docs")) return "api docs";
